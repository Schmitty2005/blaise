...
=== Set types over an integer subrange (`set of 0..255`)

`set of byte` and `set of Boolean` are *implemented* (issue #105): the named
ordinal base types lower to a bit set — `set of byte` uses the 256-bit jumbo
path, membership/Include/Exclude and `[lo..hi]` range literals all work on both
backends.  See `docs/language-rationale.adoc` and the `set of Byte` tests in
`cp.test.sets.pas` / `cp.test.e2e.sets.pas`.

What remains is the *anonymous integer-subrange* base form,
`set of 0..255` / `set of 1..10`, which is still rejected at parse time
("Expected enum type or '(' after 'set of'").  Standard Pascal accepts a
subrange there; the bit count is the range width and element ordinals are the
integer values themselves — the same machinery `set of byte` already uses, so
this is mostly parser + base-type-resolution work, not new codegen.

Minor follow-on: for `set of Boolean`, a Boolean operand on the left of `in`
(e.g. `True in s`) is rejected by the `in` type-check ("operand must be
numeric"); `Ord(b) in s` works.  The check should accept a Boolean (or any
ordinal) operand against a matching ordinal-base set.
